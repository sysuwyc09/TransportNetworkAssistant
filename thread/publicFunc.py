...
def fixCompany(text):
    '''判断归属的区域'''
    match = re.search('(赤坎|霞山|坡头|麻章|开发区|雷州|廉江|吴川|遂溪|徐闻)',text)
    if match:
        if match.group(0) == '麻章' or match.group(0) == '赤坎':
            return '城区一'
        elif match.group(0) == '霞山' or match.group(0) == '坡头':
            return '城区二'
        else:
...
"""写机房位置"""
def writePoint(name,longitude,latitude,icon='ranger_station.png',description=''):
    ...
    placeMark = placeMark + '\t'*5 + f'<href>http://maps.google.com/mapfiles/kml/shapes/{icon}</href>\n'
    placeMark = placeMark + '\t'*4 + '</Icon>\n'
    placeMark = placeMark + '\t'*4 + '<color>ff00ff00</color>\n'
...
def dfToMarkdownKnowledge(df, output_path="dify_knowledge.md", encoding="utf-8"):
    ...
    try:
        ...
        return True,f"✅ 生成成功! 文件路径：{output_path}"
    except Exception as e:
